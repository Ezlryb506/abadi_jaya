...
const FaqSection = dynamic(() => import("@/components/user-dashboard/FaqSection"), {
...
export default function UserDashboardPage() {
	...
	useEffect(() => {
		(async () => {
			...
			if (error) {
				...
				console.log("No profile data found for user, creating new record...");
	...
	// Debug customerId
	useEffect(() => {
		console.log("customerId berubah:", customerId);
	}, [customerId]);

	const handleProfileChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
		console.log("handleProfileChange dipanggil:", e.target.name, e.target.value);
		const { name, value } = e.target as any;
		setProfileForm(prev => ({ ...prev, [name]: value }));
	};

	const handleProfileSave = async (e: React.FormEvent) => {
		console.log("handleProfileSave dipanggil!");
		e.preventDefault();
		
		// Jika customerId masih null, coba fetch ulang
		if (!customerId) {
			console.log("customerId tidak ada, mencoba fetch ulang...");
			...
			if (retryData?.id) {
				...
				console.log("customerId ditemukan:", retryData.id);
			} else {
				console.log("customerId masih tidak ditemukan");
							...
							<form onSubmit={handleProfileSave} className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
								...
								<div className="md:col-span-2 flex justify-end gap-2">
									...
									<button 
										...
										onClick={() => console.log("Button submit diklik!")}
									>
